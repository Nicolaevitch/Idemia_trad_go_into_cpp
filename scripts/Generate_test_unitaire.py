...
class UnitTestGenerator:
    ...
    def generate_unittest(self, python_file: str):
        ...
        messages = [
            {"role": "system", "content": "Tu es un assistant expert en tests unitaires. Analyse le code et génère des tests pertinents en respectant la structure unittest."},
            {"role": "user", "content": f"Voici un script Python nommé {module_name}. Analyse-le et génère un fichier de test en utilisant unittest, avec des cas de test bien réfléchis et pertinents. Voici le code du fichier :\n\n{python_code}"}
        ]
        ...
        test_filename = os.path.join(test_dir, f"test_{module_name}.py")
